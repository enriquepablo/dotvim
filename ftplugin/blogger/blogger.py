...
class BloggerClient(object):
    ...
    def choose_post(self):
        ...
        raw = datetime.now().strftime('%H-%M-%S')
        category = 'diario'
        if self.post_id:
            ...
            try:
                ...
                if body_elem.tail:
                    raw = body_elem.tail + u'\n'
                else:
                    raw = u'\n'
                for t in body_elem.getchildren():
                    if t.tail:
                        raw += t.tail + u'\n'
                    else:
                        raw += u'\n'
        ...
        if os.path.exists('._index.rst.swp'):
            os.unlink('._index.rst.swp')
    ...
    def save_post(self):
        ...
        f.write(rest)
        f.close()
        retval = subprocess.call(['make', 'html'], stderr=subprocess.STDOUT, stdout=open(os.devnull, 'w'))
        if retval != 0:
